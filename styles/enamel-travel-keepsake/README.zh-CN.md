...
![珐琅拾光：小型纪念物 · 金属轮廓 · 珐琅光泽](assets/beach-artwork.webp)

## 这一页的特色

一枚有金属分隔线和轻微投影的立体小物，背景颜色来自照片。
...
```text
用 $albert-imagebook，把这张照片做成「珐琅拾光」，不加字。
```

默认生成 1080×1440 的竖版纯作品。要复现本页的上下对照排版，可以说：

```text
用「珐琅拾光」，原图在上、作品在下，各占一半，成品 1080×1440，不加字。
```
...
| 咖啡露台 | 树下街巷 |
| --- | --- |
| [![珐琅拾光：咖啡露台照片转珐琅纪念物](assets/coffee-terrace-65-artwork.webp)](assets/coffee-terrace-65-comparison.webp) | [![珐琅拾光：林荫街道照片转珐琅纪念物](assets/tree-street-65-artwork.webp)](assets/tree-street-65-comparison.webp) |
...
可以直接用中文名「珐琅拾光」、英文名「Enamel Keepsake」或风格 ID `enamel-travel-keepsake` 调用。
